 ...
 *  	=> 8개를 추가 (매개변수를 8개사용하지 않고 => 클래스로 모아서 첨부)
 ...
 */
	public void empInsert(EmpVO vo)
	{
		try
		{
			...
			//------------                         ----------
			// ''를 자동으로 붙여줘서 직접 붙일 필요가 없다. => table명, 컬럼명에는 ''사용하지 않는다.
			// MyBatis => $, #
			...
			// 실행 요청
			ps.executeUpdate(); // 자동으로 commit() 수행
		}catch(Exception ex)
		{
			ex.printStackTrace();
		}
	...
	public List<Integer> empGetMgrData()
	{
		...
		// 제네릭 안에는 반드시 클래스 형만 사용해야한다 ex>int 사용하면 안됨.(제네릭스:면접의 단골 데이터의 통일화)
		try
		{
			conn=db.getConnection();
...
//						+"AND empno=?";
			String sql="SELECT empno,ename,job,mgr,TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),(SELECT dname FROM myDept md WHERE me.deptno=md.deptno) AS dname,"
					+ "(SELECT loc FROM myDept md WHERE me.deptno=md.deptno) AS loc,(SELECT grade FROM myGrade mg WHERE me.sal BETWEEN mg.losal AND mg.hisal) AS grade "
					+ "FROM myEmp me "
					+ "WHERE empno=?";
			ps=conn.prepareStatement(sql);
			//?에 값을 채운다
