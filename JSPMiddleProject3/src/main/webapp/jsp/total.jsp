	...
	표현식) 
		  연산자
		    ...
		    비교 연산 ( == (eq), != (ne), < (lt) , > (gt) ,<= (le), >= (ge))
		      ${10 eq 10} => ${10==10}
		      *** 날짜,문자열을 포함한다 
		      		  --- (한글 => 가나다라 순 , 알파벳 =>A,B,C,D a,b,c,d 순)
		    논리 연산 ( && (and) ,|| (or) )
		    		 && => 예약 가능 날짜 
		    		 || => 예약이 불가능 날짜 
		   ...
		   core : 제어문 , URL관련 , 변수 설정 (request.setAttribute())
		   		  ...
		   		  반복문
		   		  	...
		   		  	------------
		   		  	 ...
		   		  	 out.println()
		   		  	  => <c:out value="">
		   		  	  => 자바스크립트에서 자바데이터를 받아서 출력할때 <c:out value="">
		   		  	  	  jquery => $() 
		   		  	  	  			${}
		   fmt : 변환 (날짜변환 , 숫자변환 : DecimalFormat)
		   			  -----   -----			|
		   			  SimpleDateFormat	 <fmt:fortmatNumber value="" pattern="999,999">
		   			  		|
 ...
 <%--
  ...
  <%
  	...
  	List<String> names=new ArrayList<String>();
   	...
   	request.setAttribute("names", names); // request, session에 담아야 EL 사용 가능
