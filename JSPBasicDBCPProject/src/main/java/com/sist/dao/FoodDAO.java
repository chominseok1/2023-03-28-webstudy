	...
	// 사용자 요청 => Connection 객체를 얻어온다 (미리 연결)
		 ...
		 *  <Resource
       	  ...
       	  maxWait="-1" : Connection을 다 사용시에 
       	  				 반환 대기 시간 (-무한)
       	  name="jdbc/oracle" : Connection 객체를 찾기 위한 이름 (key)
       	  					   lookup(key) => 객체 반환
...
ConnectionPool 설정
	...
	public List<FoodBean> foodFindData(int page,String fd)
	{
		...
		try
		{
			...
			// 서울특별시 강서구 마곡중앙로 161-22 B동 117호 지번 서울시 강서구 마곡동 758-2 B동 117호
			// 강서구 마곡중앙로 161-22 B동 117호 지번 서울시 강서구 마곡동 758-2 B동 117호
			// 강서구
			while(rs.next())
			{
