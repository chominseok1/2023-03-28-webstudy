...
public class StudentDAO {
	...
	// 함수(프로시저) 호출
	private CallableStatement cs; //함수 호출은 CallableStatement
	// URL
	...
	public void studentInsert(StudentVO vo)
	{
		try
		{
			...
			// 실행 요청
			cs.executeQuery(); // 무저건 cs.executeQuery()로 실행한다
		...
		finally
		{
			...
			}catch(Exception ex) {} // disConnection 메소드
		}
	}
	// 데이터 수정
...
/
	...
	public StudentVO studentDetail(int hakbun)
	{
		...
		try
		{
			...
			cs.setInt(1, hakbun);
			// out변수일때 => 메모리에 저장한다
			cs.registerOutParameter(2, OracleTypes.VARCHAR); // OUT변수일 경우에 cs.registerOutParameter() OracleTypes (oracle.jdbc)
	 ...
	 * CREATE OR REPLACE PROCEDURE studentListData(
		...
		IS
		BEGIN
    	OPEN pResult FOR --커서 열고
       		SELECT * FROM student; -- 실행결과를 위에다가 담는다
		END;
		/
	 */
	public List<StudentVO> studentListData()
	{
		...
		try
		{
			...
			cs.executeQuery();
			// 결과값을 받는다
			ResultSet rs=(ResultSet)cs.getObject(1); //object로 받아서 resultset으로 변환
			//CURSOR => ResultSet로 변환
			while(rs.next())
			{
				StudentVO vo=new StudentVO();
	...
	//중복 코드가 있는 여부 => 공통 모듈 (반복적일 때 메소드화 처리, 클래스화)
}
