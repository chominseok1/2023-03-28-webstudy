	   	  ...
	   	  request : HttpServletRequest
	   	  			...
	   	  			= 요청정보
	   	  			 ...
	   	  			 = 추가정보 (MVC,MV) => 오라클에서 받은 값을 추가 => JSP
	   	  			 	* setAttribute() : 기존에 있는 request값에 출력에 필요한 데이터를 추가
	   	  			 ...
	   	  			 -----
	   	  			  ...
	   	  			  ----------
	   	  			   필요시에 서버에서 화면을 요청 화면이 아닌 다른 화면 이동
	   	  			          ...
	   	  			          delete => list
	   	  			          update => detail
	   	  			   => sendRedirect() => GET방식 , request를 초기화 한 다음 이동
	   	  			 Header 정보 : 실제 데이터 전송 전에 처리
	   	  			   setHeader() => 한글변환 / 다운로드시에 파일명,크기 => 다운로드창
	   	  response
...
for(EmpVO vo:list)
{
    ...
    out.print("<td class=text-center><a href=\"MainServlet?mode=2&empno="+vo.getEmpno()+"\">"+vo.getEname()+"</a></td>");// 상세보기는 번호 넘김 ?empno
