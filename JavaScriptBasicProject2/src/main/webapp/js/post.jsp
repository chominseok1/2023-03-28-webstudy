<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
        ...
        request.setCharacterEncoding("UTF-8"); // 한글 안깨지게 디코딩
    	String dong=request.getParameter("dong"); //dong을 dao로보내면 dong매개변수로 데이터베이스에서 찾아줌 처음에 null
    	int count=0;
    	List<ZipcodeVO> list=null;
    	if(dong!=null) // null 이 아닐때
    	{
...
<%--
	...
	4) ***JSTL / EL
		  => 태그로 자바 라이브러리를 만들어 준다 Java Standard Tag Library 
		  => el 화면출력 => jsp <%= %> el=> ${}
...
let ok=(zip,addr)=>{
	// opener => js7.html (팝업창을 띄워주는 위치를 opener)
	// 000-000
	opener.frm.post1.value=zip.substring(0,3); //frm post1 에 값채우기 012
	opener.frm.post2.value=zip.substring(4,7); // frm post2 에 값채우기 456 
