<%--
	 	...
	 	***= MVC
	 	...
	 	-------------
	 	 ...
	 	 JSP는 항상 => 서블릿으로 변경후에 실행
	 	 			 ------------------ 톰캣에 의해 변경
	 	 JSP는 화면에 출력할 내용의 메소드인데 들어가는 재용을 코딩한다
	 	 --- 클래스가 아니고 메소드 영역이다
	 	 _jspService()
	 	 {
	 	 	----------
	 ...
	 26page
	  ...
	  3. servlet
		 ...
		 init
		   |
		  service ==> 브라우저에 요청한 내용애 대한 처리 => 응답할 HTML작성
		  	=doGet
		  	=doPost
		  |
		  destroy() ==> 파일이름/새로고침 => 자동으로 메모리 해제 (gc())
		  
	4. JSP
		|
	  _jspInit() : wxb.xml => 저장된 내용 읽어서 저장
	  			    error 코드 , 서블릿 저장, 환경설정 위치, 한글변환
	  	|
	  _jspService() : 사용자 요청 처리 결과
	    |
	  _jspDestroy() : 페이지 이동, 새로고침 (초기화)
	  			=>  request에 담기 모든정보를 잃어 버린다
