...
public class FoodJjimLikeDAO {
  ...
  public void foodJjimInsert(FoodJJimVO vo)
  {
	  try
	  {
		  ...
		  ps.setString(1, vo.getId()); //id
		  ps.setInt(2, vo.getFno()); // fno
		  ps.executeUpdate();
	  }catch(Exception ex)
	  {
		  ex.printStackTrace();
  ...
  public int foodJjimCount(String id,int fno)
  {
	  ...
	  {
		  conn=db.getConnection();
		  String sql="SELECT count(*) FROM food_jjim WHERE fno=? AND id=?";
		  ps=conn.prepareStatement(sql);
		  ps.setInt(1, fno);
		  ps.setString(2, id);
 ...
 // 찜 목록 출력 => 마이페이지 (찜관리) 
  ...
  {
	  List<FoodJJimVO> list=new ArrayList<>();
	  try
	  {
		  conn=db.getConnection();
...
//		  			+ "(SELECT tel FROM food_house WHERE fno=food_jjim.fno) "
//		  			+ "FROM food_jjim "
//		  			+ "WHERE id=? "
//		  			+ "ORDER BY no DESC";
